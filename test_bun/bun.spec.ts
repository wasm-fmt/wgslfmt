...
import init, { format } from "../pkg/wgslfmt_web.js";

await init();

const test_root = Bun.fileURLToPath(import.meta.resolve("../test_data"));

for await (const case_name of new Glob("**/*.wgsl").scan({ cwd: test_root, dot: true })) {
	...
	const snap_path = full_path.slice(0, -5) + ".snap.output";
